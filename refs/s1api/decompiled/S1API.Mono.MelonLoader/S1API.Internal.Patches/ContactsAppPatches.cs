...
internal class ContactsAppPatches
{
	...
	private static bool HasCustomNpcTypes()
	{
		...
		{
			Logger.Error($"Error in HasCustomNpcTypes: {arg}");
	...
	private static IEnumerator WaitForNPCs(ContactsApp contactsApp)
	{
		...
		if (customNPCs.Count == 0)
		{
			...
			{
				Logger.Error($"Error invoking Start: {arg}");
				yield break;
			}
		}
		yield return (object)new WaitUntil((Func<bool>)delegate
		{
			NPC[] allSceneNPCs = Object.FindObjectsOfType<NPC>(true);
			return customNPCs.All((NPC npc) => allSceneNPCs.Any((NPC sn) => sn.ID == npc.ID));
		});
		yield return (object)new WaitUntil((Func<bool>)(() => customNPCs.All((NPC npc) => npc.RelationshipDataAppliedFromPrefab)));
		yield return (object)new WaitUntil((Func<bool>)(() => customNPCs.All((NPC npc) => (Object)(object)npc.S1NPC != (Object)null && npc.S1NPC.RelationData != null)));
		yield return null;
		yield return null;
		yield return (object)new WaitUntil((Func<bool>)(() => NPCAppearance.MugshotsProcessingComplete));
		try
		{
			startMethod.Invoke(contactsApp, null);
		}
		...
		{
			Logger.Error($"Error invoking Start: {arg2}");
		}
	...
	private static void AddRelationCircles(ContactsApp contactsApp)
	{
		...
		Dictionary<EMapRegion, RegionUI> dictionary = contactsApp.RegionUIs.ToDictionary((RegionUI r) => r.Region, (RegionUI r) => r);
		...
		foreach (NPC npc in list)
		{
			...
			{
				Logger.Warning($"Skipping NPC {npc.ID} - S1NPC={(Object)(object)npc.S1NPC != (Object)null}, RelationData={npc.S1NPC?.RelationData != null}");
				continue;
			}
			if (!dictionary.TryGetValue(npc.S1NPC.Region, out var value) || (Object)(object)value?.Container == (Object)null)
			{
			...
			RelationCircle[] componentsInChildren = ((Component)value.Container).GetComponentsInChildren<RelationCircle>(true);
		...
		RegionUI[] regionUIs = contactsApp.RegionUIs;
		foreach (RegionUI val4 in regionUIs)
		{
			List<RelationCircle> value2 = (((Object)(object)val4.Container == (Object)null) ? new List<RelationCircle>() : ((Component)val4.Container).GetComponentsInChildren<RelationCircle>(true).ToList());
			dictionary3[val4.Region] = value2;
		}
		Dictionary<NPC, List<NPC>> graph = BuildGraph(list);
		HashSet<string> hashSet = new HashSet<string>();
		foreach (KeyValuePair<EMapRegion, List<RelationCircle>> item in dictionary3)
		{
			foreach (RelationCircle item2 in item.Value)
			{
				if (!string.IsNullOrEmpty(item2.AssignedNPC_ID))
				{
					hashSet.Add(item2.AssignedNPC_ID);
				}
			}
		}
		List<NPC> source = ComputeInsertionOrder(list, graph, hashSet);
		foreach (IGrouping<EMapRegion, NPC> item3 in from n in source
			group n by n.S1NPC.Region)
		{
			if (!dictionary.TryGetValue(item3.Key, out var value3) || (Object)(object)value3?.Container == (Object)null)
			{
				continue;
			}
			RelationCircle[] componentsInChildren2 = ((Component)value3.Container).GetComponentsInChildren<RelationCircle>(true);
			Dictionary<string, RelationCircle> circleById = componentsInChildren2.Where((RelationCircle c) => !string.IsNullOrEmpty(c.AssignedNPC_ID)).ToDictionary((RelationCircle c) => c.AssignedNPC_ID, (RelationCircle c) => c);
			Dictionary<RelationCircle, RectTransform> dictionary4 = componentsInChildren2.ToDictionary((RelationCircle c) => c, (RelationCircle c) => ((Component)c).GetComponent<RectTransform>());
			foreach (NPC npc2 in item3)
			{
				RelationCircle val5 = ((IEnumerable<RelationCircle>)componentsInChildren2).FirstOrDefault((Func<RelationCircle, bool>)((RelationCircle c) => c.AssignedNPC_ID == npc2.S1NPC.ID));
				if (!((Object)(object)val5 == (Object)null))
				{
					List<(Vector2, Vector2)> allEdges = GetAllEdges(componentsInChildren2, dictionary4);
					Vector2 anchoredPosition = ComputePlacement(val5, componentsInChildren2, circleById, dictionary4, allEdges);
					dictionary4[val5].anchoredPosition = anchoredPosition;
				}
			}
		}
	...
	private static void CreateConnectionLines(ContactsApp contactsApp, List<NPC> customNPCs, Dictionary<EMapRegion, RegionUI> regionUIs)
	{
		...
		foreach (NPC npc in customNPCs)
		{
			...
			if (obj == null || !regionUIs.TryGetValue(npc.S1NPC.Region, out RegionUI value) || (Object)(object)value?.Container == (Object)null)
			{
				continue;
			}
			RelationCircle[] componentsInChildren = ((Component)value.Container).GetComponentsInChildren<RelationCircle>(true);
			...
			List<NPC> connections = npc.S1NPC.RelationData.Connections;
			foreach (NPC connectedNPC in connections)
			{
	...
	private static Dictionary<NPC, List<NPC>> BuildGraph(List<NPC> customs)
	{
		...
		foreach (NPC custom in customs)
		{
			...
			List<NPC> list = ((relationData != null) ? relationData.Connections : null);
			if (list == null)
			{
				continue;
			}
			...
			{
				if (!((Object)(object)item == (Object)null) && item.ID != null && dictionary.TryGetValue(item.ID, out var value))
				{
	...
	private static List<NPC> ComputeInsertionOrder(List<NPC> customs, Dictionary<NPC, List<NPC>> graph, HashSet<string> nativeIds)
	{
		...
		foreach (NPC custom in customs)
		{
			NPCRelationData relationData = custom.S1NPC.RelationData;
			List<NPC> list = ((relationData != null) ? relationData.Connections : null);
			if (list != null && list.Any((NPC c) => (Object)(object)c != (Object)null && nativeIds.Contains(c.ID)))
			{
				dist[custom] = 0;
	...
	private static List<Vector2> GetConnectionPositions(RelationCircle circle, Dictionary<string, RelationCircle> circleById, Dictionary<RelationCircle, RectTransform> rectTransforms)
	{
		...
		{
			if (!((Object)(object)item == (Object)null) && !string.IsNullOrEmpty(item.ID) && circleById.TryGetValue(item.ID, out RelationCircle value))
			{
	...
	private static List<(Vector2 from, Vector2 to)> GetAllEdges(RelationCircle[] regionCircles, Dictionary<RelationCircle, RectTransform> rectTransforms)
	{
		...
		foreach (RelationCircle val in regionCircles)
		{
			...
			foreach (NPC connection in val.AssignedNPC.RelationData.Connections)
			{
				if (!((Object)(object)connection == (Object)null) && !string.IsNullOrEmpty(connection.ID) && dictionary.TryGetValue(connection.ID, out var value))
				{
	...
	private static void ContactsDetailPanel_Open_Postfix(ContactsDetailPanel __instance)
	{
		...
		{
			Debug.LogWarning((object)"Connections Text Component not found!");
			return;
		}
		...
		List<NPC> list = (List<NPC>)obj2;
		if (list == null || list.Count == 0)
		{
			component.text = "Connections: None";
			return;
		}
		string text = string.Join(", ", list.Where((NPC c) => (Object)(object)c != (Object)null).Select(delegate(NPC c)
		{
