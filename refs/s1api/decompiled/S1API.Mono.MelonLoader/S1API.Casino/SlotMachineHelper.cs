...
public static class SlotMachineHelper
{
	...
	{
		if ((Object)(object)npc?.S1NPC?.Inventory == (Object)null)
		{
			return 0f;
		}
		try
	...
	{
		if ((Object)(object)npc?.S1NPC?.Inventory == (Object)null || amount <= 0)
		{
			return false;
		}
		try
	...
	public static void AddNPCCash(NPC npc, int amount)
	{
		if ((Object)(object)npc?.S1NPC?.Inventory == (Object)null || amount <= 0)
		{
			Logger.Warning(string.Format("AddNPCCash: Invalid parameters - NPC={0}, Amount={1}, Inventory={2}", npc?.ID, amount, ((Object)(object)npc?.S1NPC?.Inventory != (Object)null) ? "Valid" : "Null"));
			return;
		}
		try
		{
	...
	public static bool UseSlotMachine(NPC npc, Vector3 machinePosition, int betAmount, float maxSearchDistance = 5f)
	{
		...
		try
		{
			...
			ESymbol[] array = (ESymbol[])(object)new ESymbol[val.Reels.Length];
			for (int i = 0; i < array.Length; i++)
			{
				array[i] = (ESymbol)(int)SlotMachine.GetRandomSymbol();
			}
			((MonoBehaviour)Singleton<CoroutineService>.Instance).StartCoroutine(SpinSlotMachineForNPC(npc, val, array, betAmount));
			return true;
		}
		catch (Exception ex)
		{
	...
	public static SlotMachine FindNearestSlotMachine(Vector3 position, float maxDistance)
	{
		...
		try
		{
			SlotMachine[] array = Object.FindObjectsOfType<SlotMachine>();
			...
			SlotMachine[] array2 = array;
			foreach (SlotMachine val in array2)
			{
	...
	private static IEnumerator SpinSlotMachineForNPC(NPC npc, SlotMachine machine, ESymbol[] symbols, int betAmount)
	{
		...
		for (int j = 0; j < machine.Reels.Length; j++)
		{
			if (j == machine.Reels.Length - 1 && (int)outcome != 0 && symbols.Length >= 3 && (int)symbols[j - 1] == (int)symbols[j - 2])
			{
			...
			machine.Reels[j].Stop(symbols[j]);
		}
