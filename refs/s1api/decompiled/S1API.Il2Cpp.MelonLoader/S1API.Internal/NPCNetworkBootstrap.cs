...
internal static class NPCNetworkBootstrap
{
	...
	private static bool HasRemoteClients()
	{
		try
		{
			...
			Dictionary<int, NetworkConnection> val = ((serverManager != null) ? serverManager.Clients : null);
			if (val == null)
			{
				return false;
			}
			int num = 0;
			Enumerator<int, NetworkConnection> enumerator = val.GetEnumerator();
			while (enumerator.MoveNext())
			{
				KeyValuePair<int, NetworkConnection> current = enumerator.Current;
				NetworkConnection value = current.Value;
	...
	private static bool AreRemoteConnectionsReady(NetworkManager nm, out bool hasRemoteClients)
	{
		...
		try
		{
			...
			Enumerator<int, NetworkConnection> enumerator = clients.GetEnumerator();
			while (enumerator.MoveNext())
			{
				KeyValuePair<int, NetworkConnection> current = enumerator.Current;
				NetworkConnection value = current.Value;
