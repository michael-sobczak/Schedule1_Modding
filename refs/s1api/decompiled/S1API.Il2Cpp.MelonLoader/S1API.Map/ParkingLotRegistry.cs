...
public static class ParkingLotRegistry
{
	...
	public static ParkingSpotWrapper[] GetFreeSpots(string lotGuid)
	{
		...
		try
		{
			...
			Enumerator<ParkingSpot> enumerator = freeParkingSpots.GetEnumerator();
			while (enumerator.MoveNext())
			{
				ParkingSpot current = enumerator.Current;
				list.Add(new ParkingSpotWrapper(current));
			}
			return list.ToArray();
		}
		catch
	...
	internal static void Register(object gameParkingLot)
	{
		...
		try
		{
			...
			string guid = ((object)val.GUID/*cast due to .constrained prefix*/).ToString();
