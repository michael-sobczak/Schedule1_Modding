	...
	private static string TryResolveNPCIdFromType(Type npcType)
	{
		...
		try
		{
			...
			if (nPCRegistry != null)
			{
				...
				{
					Enumerator<NPC> enumerator2 = nPCRegistry.GetEnumerator();
					while (enumerator2.MoveNext())
					{
						NPC current2 = enumerator2.Current;
						if ((Object)(object)current2 != (Object)null && string.Equals(current2.ID, item, StringComparison.OrdinalIgnoreCase))
						{
							return item;
						}
					}
			...
			if (_connectionIDs.Count > 0)
			{
				...
				if (connections != null)
				{
					...
					for (int i = 0; i < _connectionIDs.Count; i++)
					{
						...
						NPC val = null;
						Enumerator<NPC> enumerator = nPCRegistry.GetEnumerator();
						while (enumerator.MoveNext())
						{
							NPC current = enumerator.Current;
							if ((Object)(object)current != (Object)null && current != owner && string.Equals(current.ID, text, StringComparison.OrdinalIgnoreCase))
							{
								val = current;
								break;
							}
						...
						{
							num2++;
							Logger.Warning($"[Relationship Data] ApplyTo: Connection ID '{text}' not found in NPC registry for NPC '{((owner != null) ? owner.ID : null) ?? "<null>"}'");
						}
					}
				}
				else
				{
					Logger.Warning("[Relationship Data] ApplyTo: Connections list is null for NPC '" + (((owner != null) ? owner.ID : null) ?? "<null>") + "'");
				}
			}
		}
		catch (Exception ex)
		{
			Logger.Error("[Relationship Data] ApplyTo: Exception applying connections to NPC '" + (((owner != null) ? owner.ID : null) ?? "<null>") + "': " + ex.Message);
		}
		try
		{
