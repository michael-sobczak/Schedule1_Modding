...
public sealed class NPCDialogue
{
	...
	internal void RebuildRuntimeModules(DialogueDatabase db)
	{
		...
		Il2CppArrayBase<DialogueModule> components = ((Component)Handler).gameObject.GetComponents<DialogueModule>();
	...
	private void EnsureRuntimeModulesList()
	{
		try
		{
			...
			{
				Handler.runtimeModules = new List<DialogueModule>();
			}
		}
		catch
		{
	...
	public void BuildAndRegisterContainer(string containerName, Action<DialogueContainerBuilder> configure)
	{
		...
		List<DialogueContainer> dialogueContainers = Handler.dialogueContainers;
		if (dialogueContainers == null)
		{
			return;
		}
		int num = -1;
		...
		{
			DialogueContainer val2 = dialogueContainers[i];
		...
		if (num >= 0)
		{
			dialogueContainers[num] = val;
		}
		else
		{
			dialogueContainers.Add(val);
		}
	}
