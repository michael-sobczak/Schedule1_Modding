	...
	public void EnsureInitialized()
	{
		...
		if (count < slotCount)
		{
			...
			for (int j = 0; j < num4; j++)
			{
				...
				val4.onItemDataChanged = (Action)Delegate.Combine((Delegate)(object)val4.onItemDataChanged, (Delegate)(object)Action.op_Implicit(action));
				val4.SetSlotOwner(((Il2CppObjectBase)inv).Cast<IItemSlotOwner>());
				try
				{
		...
		{
			object? obj7 = NPC.GetType().GetMethod("GetPrimaryInteractable", BindingFlags.Instance | BindingFlags.NonPublic)?.Invoke(NPC, null);
			InteractableObject val5 = (InteractableObject)((obj7 is InteractableObject) ? obj7 : null);
			Il2CppArrayBase<InteractableObject> componentsInChildren = NPC.gameObject.GetComponentsInChildren<InteractableObject>(true);
		...
		try
		{
			object? obj9 = ReflectionUtils.TryGetFieldOrProperty(inv, "onContentsChanged");
			UnityEvent val7 = (UnityEvent)((obj9 is UnityEvent) ? obj9 : null);
			if (val7 == null)
			{
